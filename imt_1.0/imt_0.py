...
from utils import SA_Block, CA_Block, Mlp, Position_encoder, Instance_encoder, quat2mat
...
class IMT(nn.Module):
    def __init__(self, in_feature=7, out_feature=32, num_heads=8):
        ...
        # self.mlp_axis = Mlp(in_feature=out_feature, out_feature=3)
        # self.mlp_rot = Mlp(in_feature=out_feature, out_feature=1)
        self.mlp_r = Mlp(in_feature=out_feature, out_feature=4)
        self.mlp_tr = Mlp(in_feature=out_feature, out_feature=3)
    ...
    def forward(self, x1, x2):
        ...
        x1 = self.norm1(x1)
        x2 = self.norm2(x2)
        x = torch.cat([x1, x2], dim=1)  # B, m+n, C
        # axis = self.mlp_axis(x)
        # axis = torch.max(axis, dim=1, keepdim=False)[0]
        r = self.mlp_r(x)
        r = torch.max(r, dim=1, keepdim=False)[0]
        r = r / torch.norm(r, p=2, dim=1, keepdim=True)
        r = quat2mat(r)
        tr = self.mlp_tr(x)
        tr = torch.max(tr, dim=1, keepdim=False)[0]
        # rot = self.mlp_rot(x)
