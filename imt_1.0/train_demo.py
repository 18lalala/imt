...
def main(args):
    ...
    experiment_dir = Path('./log_demo_the/')
    ...
    log_dir = experiment_dir.joinpath('logs_demo_the/')
    log_dir.mkdir(exist_ok=True)
    writer = SummaryWriter('./log_demo_the/tblog_demo_the')
    ...
    root = '/media/puzek/sda/dataset/imt_data/dataset_demo/'
    ...
    '''MODEL LOADING'''
    shutil.copy('train_demo.py', str(experiment_dir))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (source, target, pose) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            axis, rot, tr = model(source, target)
            # print(pose_gt)
            loss, loss_ax, loss_the, loss_tr = criterion(pose_gt, axis, rot, tr)
        ...
        with torch.no_grad():
            ...
            for i, (source, target, pose) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                ...
                axis, rot, tr = model(source, target)
                # print(pose_gt)
                loss_val, loss_ax_val, loss_the_val, loss_tr_val = criterion(pose_gt, axis, rot, tr)

                loss_sum_val += loss_val
                loss_ax_sum_val += loss_ax_val
                loss_the_sum_val += loss_the_val
                loss_tr_sum_val += loss_tr_val

            writer.add_scalar('loss_test', float(loss_sum_val/num_batches), epoch)
            writer.add_scalar('loss_ax_test', float(loss_ax_sum_val/num_batches), epoch)
            writer.add_scalar('loss_the_test', float(loss_the_sum_val/num_batches), epoch)
