...
from dataset import collate_fn, imtdataset, collate_fn_geo, imtdataset_geo
import torch
import datetime
import logging
...
import numpy as np
import time
from utils import get_loss, get_loss_geo
from imt import IMT
...
def parse_args():
    ...
    parser.add_argument('--epoch', default=2000, type=int, help='Epoch to run [default: 200]')
...
def main(args):
    ...
    experiment_dir = Path('./log_loss_256_seed35_low/')
    ...
    root = '/media/puzek/8f930508-7c2e-449d-b227-fdbbaffbd0e7/dataset/imt_data/dataset_low/sequences/'
    BATCH_SIZE = args.batch_size

    torch.manual_seed(35)

    print("start loading training data ...")
    TRAIN_DATASET = imtdataset_geo(split='train', data_root=root, test_area=args.test_area)
    print("start loading test data ...")
    TEST_DATASET = imtdataset_geo(split='test', data_root=root, test_area=args.test_area)
    ...
    testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=BATCH_SIZE, shuffle=False, num_workers=10,
                                                 pin_memory=True, drop_last=True,collate_fn=collate_fn_geo)
    #testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=BATCH_SIZE, shuffle=False, num_workers=10,
    #                                             pin_memory=True, drop_last=True)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (source, target, pose, source_pt) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            source = source.data.numpy()
            target = target.data.numpy()
            pose_gt = pose.data.numpy()
            source_pt = source_pt.data.numpy()
            source = torch.Tensor(source)
            target = torch.Tensor(target)
            pose_gt = torch.Tensor(pose_gt)
            source_pt = torch.Tensor(source_pt)
            source, target, pose_gt, source_pt= source.float().cuda(), target.float().cuda(), pose_gt.float().cuda(), source_pt.float().cuda()

            rot, tr = model(source, target)
            # print(pose_gt)
            loss, loss_rot, loss_tr, loss_geo = criterion(pose_gt, source_pt, rot, tr)
        ...
        writer.add_scalar('loss_geo_train', float(loss_geo_sum/num_batches), epoch)
        ...
        with torch.no_grad():
            ...
            for i, (source, target, pose, source_pt) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                source = source.data.numpy()
                target = target.data.numpy()
                pose_gt = pose.data.numpy()
                source_pt = source_pt.data.numpy()
                ...
                source_pt = source_pt.float().cuda()
                rot, tr = model(source, target)
                # print(pose_gt)
                loss_val, loss_rot_val, loss_tr_val, loss_geo_val = criterion(pose_gt, source_pt, rot, tr)

                loss_sum_val += loss_val
                loss_rot_sum_val += loss_rot_val
                loss_tr_sum_val += loss_tr_val
                loss_geo_sum_val += loss_geo_val
            ...
            writer.add_scalar('loss_geo_test', float(loss_geo_sum_val/num_batches), epoch)
